...
"""Kubernetes API helper for the sigul PKI bootstrap Job.
...
Talks to the API server using the pod's ServiceAccount token.
...
Usage:
    ...
    publish-secrets.py get NAME KEY
        -> print the decoded value of KEY, or exit 1 if the Secret
...
def main() -> int:
    ...
    cmd = str(args.cmd)  # pyright: ignore[reportAny]
    if cmd == "exists":
    ...
    if cmd == "get":
        value = get_secret_key(str(args.name), str(args.key))  # pyright: ignore[reportAny]
        if value is None:
            return 1
        print(value)
        return 0
