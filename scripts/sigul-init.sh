...
#   SIGUL_BRIDGE_HOST      - Bridge host binding (default: 0.0.0.0)
#   SIGUL_BRIDGE_PORT      - Bridge port (default: 44334)
...
setup_env() {
    ...
    export SIGUL_BRIDGE_HOST="${SIGUL_BRIDGE_HOST:-0.0.0.0}"
    export SIGUL_BRIDGE_PORT="${SIGUL_BRIDGE_PORT:-44334}"

    # Validate numeric ports
    if ! [[ "$SIGUL_BRIDGE_PORT" =~ ^[0-9]+$ ]] || (( SIGUL_BRIDGE_PORT < 1 || SIGUL_BRIDGE_PORT > 65535 )); then
        error "Invalid SIGUL_BRIDGE_PORT: $SIGUL_BRIDGE_PORT. Must be 1-65535." $EXIT_INVALID_ARGS
    fi
...
generate_server_config() {
    ...
    export SIGUL_BRIDGE_HOST="${SIGUL_BRIDGE_HOST:-sigul-bridge}"
    export SIGUL_BRIDGE_PORT="${SIGUL_BRIDGE_PORT:-44334}"
...
host = ${SIGUL_BRIDGE_HOST}
port = ${SIGUL_BRIDGE_PORT}
...
[client]
# Bridge connection configuration
bridge-hostname = ${SIGUL_BRIDGE_HOST}
bridge-port = ${SIGUL_BRIDGE_PORT}
...
start_bridge() {
    ...
    debug "  Expected log file: $log_file"
    debug "  Bridge host: ${SIGUL_BRIDGE_HOST:-0.0.0.0}"
    debug "  Bridge port: ${SIGUL_BRIDGE_PORT:-44334}"
